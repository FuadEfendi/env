...
is_server_running()
{
    SrvrPid=`$PS $PS_OPTS |grep "java.*-Dweblogic.Name=$ServerName.*weblogic.Server" |grep -v "grep" |awk '{print $2}'`
    if [ -z $SrvrPid ]; then
...
is_lockfile_correct() {
    if read_file "$LockFile"; then
        $PS $PS_OPTS |grep "$REPLY" |grep -v "grep" |grep "wlscontrol.sh" > $NullDevice 2>&1
        if [ $? -ne 0 ]; then
...
start_and_monitor_server()
{
    ...
    while true; do
        ...
        read_file "$StateFile"
    	case $REPLY in
            *:N:*)  print_info "Server startup failed and will be retried"
                startupFailed=true
		            ;;
            FAILED:*:N)  print_info "Server failed during shutting down"
        ...
        compute_diff_time
        #Note that this logic is not applied for startup failure to
        #avoid infinite process startup loop
        if [ $diff_time -gt $RestartInterval ] && [ "$startupFailed" != "true" ]
        then
            #Reset count
            #need to set to 1 so that we don't restart an extra
            #time.
            count=1
            LastBaseStartTime=0
        fi
        if [ $AutoRestart != true ]; then
            print_err "Server failed but is not restartable because auto "\
                      "restart is disabled."
            if [ "$startupFailed" != "true" ]; then
              write_state FAILED_NOT_RESTARTABLE:Y:N
            else
             write_state FAILED_NOT_RESTARTABLE:N:N
            fi
	        return 1
...
#
# Process node manager PRINTTHREADDUMP command to send SIGQUIT the currently running server.
# Returns true if successful otherwise returns false if the server process
# was not running or could not be killed.
#
do_printthreaddump()
{
    ...
    # Check for pid file
    if [ x$srvr_pid = x ]; then
      echo "Could not send SIGQUIT to server process (pid file not found).  Make sure that the nodemanager native library is supported on the platform and can be loaded using the library path specified for the server" >&2
      return 1
    fi
    # Kill the server process
    kill -QUIT $srvr_pid
...
#
    KILL)   do_kill ;;
    PRINTTHREADDUMP)   do_printthreaddump ;;
    ...
    if [ -d  $DomainDir/servers ]; then
      ...
      do
        ...
        if [ -d $DomainDir/servers/$ServerName/data/nodemanager ]; then
          ...
          result="${result}${ServerName}=${state}${separator}"
        fi
     done
   fi
    InternalStatCall=N
    # remove the last char if it is separator
    lastChar="${result: -1}"
    if [ $lastChar == ${separator} ]; then
...
do_validate_separator()
{
  string=$1
  check=${#string}
  if [ ! -z "${string// }" ] && [ $check -gt 1 ]; then
    print_err "Separator must be a single character, or empty for space by default" 2>&1
  fi
}
...
FullVersion="NodeManager version 12.2.1.4.0"
DomainDir=
DomainName=
ServerName=
AutoRestart=true
