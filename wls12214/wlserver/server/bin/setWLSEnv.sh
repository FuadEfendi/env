...
# (@DOCSWEBROOT/START/overview.htm).
#*****************************************************************************
...
CLASSPATH="${FMWCONFIG_CLASSPATH}${CLASSPATHSEP}${CLASSPATH}"
export CLASSPATH
...
if [ ! -d "${M2_HOME}" ]; then
  M2_HOME=${MW_HOME}/oracle_common/modules/org.apache.maven_3.2.5
fi

PATH="${PATH}${PATHSEP}${M2_HOME}/bin"
