...
WL_HOME="c:/Users/xiaol/Downloads/wls/wls12214/wlserver"
...
restoreOrigEnv() {
 ...
 for line in $pre ; do
   ...
   if [ "$name" != "JAVA_HOME" ] && [ "$name" != "SUN_JAVA_HOME" ] && [ "$name" != "DEFAULT_SUN_JAVA_HOME" ] && [ "${!name}" != "${value}" ]; then
     echo "$name" | grep -q -E "$excluderegex"
     if [ $? != 0 ];then
...
EXTRA_NM_LIBS="${WL_HOME}/modules/features/oracle.wls.common.grizzly.jar"

CLASSPATH="${WEBLOGIC_CLASSPATH}${CLASSPATHSEP}${CLASSPATH}${CLASSPATHSEP}${BEA_HOME}${CLASSPATHSEP}${EXTRA_NM_LIBS}"
